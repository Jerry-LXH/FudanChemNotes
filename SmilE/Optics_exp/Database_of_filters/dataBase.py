...
class FilterDatabaseSQLite:
    """An object that connects/creates a SQLITE database and offers methods to manipulate the tables stored there. It's recommended to store the original data here."""
    ...
    def update_notes(self, name, new_notes):
        """Update notes based on name."""
        cursor = self.conn.cursor()
        cursor.execute("UPDATE filters SET notes=? WHERE name=?", (new_notes, name))
        self.conn.commit()
        if cursor.rowcount == 0:
            print(f'No filter is named {name}.')
        else:
            print(f"Filter {name} is noted: {new_notes}")
    ...
    def delete_filter(self, name):
        ...
        self.conn.commit()
        print(f"Filter {name} is deleted.")

    def list_all_filters(self, order_by="manufacturer"):
        """List all filters by manufacturer or type."""
        cursor = self.conn.cursor()
